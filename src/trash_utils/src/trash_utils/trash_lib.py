...
def trash_2d_gauss(mu, sigma, num_soda_cans, dfunc):
    ...
    Inputs:
        mu (list)           : x,y mean of the normal distribution. 
                              Set to the center of the point
...
def init_hotspots(trash_x_centers, trash_y_centers, input_sigma, num_soda_cans, dfunc):
    ...
    Inputs:
        ...
        input_sigma (array)          : (Optional) If provided, used to determine the 
                                        standard deviation of the spread for all of the
                                        trash hotspots
        num_soda_cans (int)          : number of hotspots
        dfunc (function)             : depth function created from finder_utils

    Returns:
        trash_dict (Dict)            : keys = soda can ID number, 
                                       items = np.ndarray of soda can's positions 
...
def update_trash_pos(trash_dict, time_now_hrs, e, tracking=False):
    '''
    Takes dict of trash positions. 
    Updates them according to the currents + noise and random walk

    Inputs:
        trash_dict (Dict)   : keys = soda can ID number, 
                              items = np.ndarray of soda can's positions throughout the 
                              timesteps
        time_now_hrs (float): time to update at in hrs
        e (Object)          : Object (Env.py) contains all the environment properties 
                              (currents, height, width)
    ...
    for group_ID in trash_dict.keys():
        ...
        if random_walk_chance < 0.8:
            rearrange_cans = np.hstack((cans[:,0].reshape(-1,1), 
                                        cans[:,-1].reshape(-1,1), 
                                        cans[:,1:3]))
            local_u = e.ufunc(rearrange_cans) * np.random.normal(1, 0.5)
            local_v = e.vfunc(rearrange_cans) * np.random.normal(1, 0.5)
        else:
            random_range = np.random.uniform(0, 2)
            local_u = np.random.uniform(-random_range, random_range, 
                                        size=(cans.shape[0], 1))
            local_v = np.random.uniform(-random_range, random_range, 
                                        size=(cans.shape[0], 1))

        ##Move the soda cans by that amount (not technically correct, but an approximation)
        cans[:, 1] += local_u.flatten()
        cans[:, 2] += local_v.flatten()

        ##TODO: TEST THIS
        ##If the position of the soda can is at the bounds, 
        ##then set the setoda can position to boundary value
        cans[:,1][cans[:,1] > e.width/2] = e.width/2
        cans[:,1][cans[:,1] < -e.width/2] = -e.width/2
...
def visualize_trash_flow(trash_dict, *args):
    '''
    ...
    Inputs:
        trash_dict (Dict)   : keys = soda can ID number, 
                              ...
                              timesteps
    *args in order: 
        vis_clusters (bool) : True = draws convex hull + buffer around each trash hotspot 
        p (matplotlib fig)  : matplotlib plt.figure() or subplot to plot trash pos on
    ...
    for trash_group_ID in trash_dict.keys():
        ...
        for trash_piece in range(group.shape[1]):
            ...
            p.plot(movement[:, trash_piece, 0], movement[:, trash_piece, 1], color=group_color, marker='_', linewidth=2)

        if vis_clusters:
            convexhull_a = sg.MultiPoint(group[-1][:, 0:2]).convex_hull
            buffer_a = convexhull_a.buffer(5)
            ax, ay = buffer_a.exterior.coords.xy
            p.plot(ax, ay, color=group_color)


def visualize_trash_step(trash_dict, *args):
    '''
    ...
    *args in order: 
    ...
    for trash_group_ID in trash_dict.keys():
        ...
        for trash_piece in range(group.shape[1]):
            movement = np.array(trash_dict[trash_group_ID])
            p.plot(movement[-1, trash_piece, 0], movement[-1, trash_piece, 1], color=group_color, marker='o')
